...
import torch.hub  # Import torch.hub for model loading
...
class EnhancedAnimeRanker(nn.Module):
    def __init__(self):
        ...
        # Сохраняем AdaptiveConcatPool2d и Flatten, удаляем остальное
...
class ListNetLoss(nn.Module):
    def __init__(self, label_smoothing=0.05, epsilon=1e-5):
        super().__init__()
...
class CombinedLoss(nn.Module):
    def __init__(self, listnet_weight=0.1, ranknet_weight=0.9, margin=0.5):
        super().__init__()
...
def visualize_predictions(model, dataset, num_examples=5):
    ...
    for plot_idx, data_idx in enumerate(indices):
        ...
        for i in range(5):
            ...
            img = img * [0.2970, 0.3017, 0.3017] + [0.7137, 0.6628, 0.6519]
...
def train():
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(train_val_groups)):
        ...
        optimizer = torch.optim.AdamW([
            {'params': last_block.parameters(), 'lr': CFG['head_lr'] / 10},  # Меньший LR для backbone
        ...
        criterion = CombinedLoss(listnet_weight=0.1, ranknet_weight=0.9, margin=0.5)
            ...
            for images, labels in progress_bar:
                ...
                if torch.isnan(head_grad_norm) or torch.isinf(head_grad_norm) or \
                   torch.isnan(backbone_grad_norm) or torch.isinf(backbone_grad_norm):
                    print(f"Warning: NaN or Inf gradient detected in epoch {epoch+1}, batch {progress_bar.n}")
                    scaler.update()
                ...
                epoch_head_grad_norm += head_grad_norm.item() if not (torch.isnan(head_grad_norm) or torch.isinf(head_grad_norm)) else 0
                epoch_backbone_grad_norm += backbone_grad_norm.item() if not (torch.isnan(backbone_grad_norm) or torch.isinf(backbone_grad_norm)) else 0
    ...
    # Split train_val_groups into final train and validation for monitoring
    ...
    plt.subplot(1, 2, 2)
    plt.plot(final_history['val_ndcg'], label='Validation NDCG')
    plt.plot(final_history['val_top1'], label='Validation Top-1')
    plt.plot(final_history['swa_val_ndcg'], label='SWA Validation NDCG', linestyle='--')
    plt.plot(final_history['swa_val_top1'], label='SWA Validation Top-1', linestyle='--')
    ...
    # Visualize predictions for the best final model
    final_model = EnhancedAnimeRanker().to(CFG['device'])
