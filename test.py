...
# Загрузка списка тегов Danbooru
...
def my_collate_fn(batch):
    ...
    best_text_emb = torch.stack([item[4] for item in batch])
    other_text_emb = torch.stack([item[5] for item in batch])
    return best_images, other_images, targets, group_names, best_text_emb, other_text_emb
...
class AnimeGroupDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if np.any(np.isnan(best_img_np)) or np.any(np.isinf(best_img_np)):
            print(f"Предупреждение: NaN или Inf в лучшем изображении {best_img_path}")
            best_img_np = np.zeros_like(best_img_np)
        
        other_img_path = os.path.join(group_path, png_files[other_idx])
        other_img = Image.open(other_img_path).convert('RGB')
        other_img_np = np.array(other_img)
        if np.any(np.isnan(other_img_np)) or np.any(np.isinf(other_img_np)):
        ...
        if self.transform:
            ...
            other_img = self.transform(image=other_img_np)['image']
        
        return best_img, other_img, torch.tensor(target, dtype=torch.float32), group, best_text_emb, other_text_emb
...
class EnhancedAnimeRanker(nn.Module):
    def __init__(self):
        ...
        feature_size = 4096 + 384  # 4096 (ResNet) + 384 (SentenceTransformer)
    ...
    def forward(self, x, text_emb):
        ...
        combined = torch.cat([features, text_emb], dim=-1)
        scores = torch.stack([self.rank_head(combined[i]) for i in range(batch_size)], dim=0)
...
def evaluate(model, dataloader, device):
    ...
    with torch.no_grad():
        group_dict = {}
        for batch_idx, (best_images, other_images, _, group_names, best_text_emb, other_text_emb) in enumerate(dataloader):
            for i in range(best_images.size(0)):
                group_id = group_names[i]
                if group_id not in group_dict:
                    group_dict[group_id] = []
                group_dict[group_id].append((best_images[i:i+1], best_text_emb[i:i+1]))
                group_dict[group_id].append((other_images[i:i+1], other_text_emb[i:i+1]))
        ...
        for group_id, images in group_dict.items():
            ...
            group_text_emb = torch.cat(group_text_emb[:5], dim=0).to(device)
            ...
            if group_images.shape[1] != 3:
                ...
                continue
            scores = model(group_images.unsqueeze(0), group_text_emb.unsqueeze(0))
...
def visualize_predictions(model, dataset, num_examples=5):
    ...
    for plot_idx, data_idx in enumerate(indices):
        best_img, other_img, _, group, best_text_emb, other_text_emb = dataset[data_idx]
        group_images = torch.stack([best_img, other_img, other_img, other_img, other_img])
        group_text_emb = torch.stack([best_text_emb, other_text_emb, other_text_emb, other_text_emb, other_text_emb])
        with torch.no_grad():
            scores = model(group_images.unsqueeze(0).to(CFG['device']), group_text_emb.to(CFG['device'])).cpu().numpy()[0]
...
def train():
    ...
    for epoch in range(CFG['epochs']):
        ...
        for batch_idx, (best_images, other_images, targets, _, best_text_emb, other_text_emb) in enumerate(progress_bar):
            ...
            best_text_emb = best_text_emb.to(CFG['device'], non_blocking=True)
            other_text_emb = other_text_emb.to(CFG['device'], non_blocking=True)
            ...
            scores_best = model(best_images.unsqueeze(1), best_text_emb).squeeze(-1)
            scores_other = model(other_images.unsqueeze(1), other_text_emb).squeeze(-1)
