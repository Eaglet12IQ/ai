...
val_transform = A.Compose([
    ...
    A.CenterCrop(CFG['img_size'], CFG['img_size']),   # или height, width — здесь два числа нормально
...
def evaluate(model, dataloader, device):
    ...
    top2_correct = 0
    ndcg_scores = []
    val_loss_sum = 0.0          # ← добавлено
    val_loss_count = 0          # ← добавлено
    processed_groups = 0
    
    criterion = SoftFocalPairwiseLoss()  # ← добавлено для валидационного лосса
    
    with torch.no_grad():
        group_images_dict = {}
        
        # === Сбор всех изображений по группам ===
        for best_images, other_images, targets, group_names in dataloader:
            ...
            # Расчёт валидационного лосса на этом батче
            ...
            val_loss_sum += batch_loss.item() * best_images.size(0)   # взвешиваем по размеру батча
        ...
        # === Оценка каждой группы (метрики ранжирования) ===
    ...
    return {
        ...
        'ndcg': mean_ndcg,
        'val_loss': avg_val_loss,      # ← добавлено
...
def train():
    ...
    for epoch in range(CFG['epochs']):
        ...
        print(f"Val Loss: {val_metrics['val_loss']:.4f}")           # ← добавлено
